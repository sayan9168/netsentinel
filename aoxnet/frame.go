...
func Decode(r *bufio.Reader, maxPayload uint32) (Frame, error) {
	...
	if binary.BigEndian.Uint32(h[0:4]) != Magic {
		return Frame{}, errors.New("invalid AOXNet magic")
	}
	if h[4] != Version {
		return Frame{}, fmt.Errorf("unsupported protocol version: %d", h[4])
	}
	n := binary.BigEndian.Uint32(h[20:24])
	if n > maxPayload {
		return Frame{}, fmt.Errorf("payload exceeds limit: %d", n)
	}
	payload := make([]byte, n)
	...
	if binary.BigEndian.Uint32(trailer[:]) != crc.Sum32() {
		return Frame{}, errors.New("CRC32 mismatch")
	}
