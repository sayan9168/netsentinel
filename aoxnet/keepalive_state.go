...
func (s *KeepaliveState) Sent(id uint64, now time.Time) {
	s.mu.Lock()
	s.pending[id] = now
	s.mu.Unlock()
}
func (s *KeepaliveState) Ack(id uint64) {
	s.mu.Lock()
	delete(s.pending, id)
	s.missed = 0
	s.mu.Unlock()
...
func RunKeepalive(ctx context.Context, cfg KeepaliveConfig, probe func() (uint64, error), state *KeepaliveState, onDead func()) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case now := <-ticker.C:
			if state.Expired(now, cfg.Timeout) >= cfg.MaxMissed {
				onDead()
				return
			}
			if id, err := probe(); err == nil {
				state.Sent(id, now)
			}
		}
	}
