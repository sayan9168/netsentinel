...
func ValidateTLSConfig(cfg SecurityConfig) error {
	...
	if cfg.TLSConfig == nil {
		return errors.New("TLS is required but no TLS config was provided")
	}
	if cfg.RequireClientCertificate && cfg.TLSConfig.ClientAuth == tls.NoClientCert {
		return errors.New("client certificate authentication is required")
	}
	return nil
}
...
func PeerIdentityFromState(state tls.ConnectionState) (PeerIdentity, error) {
	if len(state.PeerCertificates) == 0 {
		return PeerIdentity{}, errors.New("peer presented no certificate")
	}
...
func HandshakeDeadline(d time.Duration) error {
	if d < 0 {
		return fmt.Errorf("invalid handshake deadline: %s", d)
	}
	return nil
}
