...
func (s *Stream) Open() error {
	...
	if s.State != StreamIdle {
		return errors.New("stream is not idle")
	}
	s.State = StreamOpen
	return nil
}
...
func (s *Stream) ConsumeSendWindow(n int64) error {
	...
	if s.State == StreamClosed || s.State == StreamHalfClosedLocal {
		return errors.New("stream is not writable")
	}
	if n > s.SendWin {
		return fmt.Errorf("flow-control window exceeded: need=%d available=%d", n, s.SendWin)
	}
	s.SendWin -= n
	return nil
}
...
func (s *Stream) AddSendWindow(n int64) error {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.SendWin > MaxWindow-n {
		return errors.New("flow-control window overflow")
	}
	s.SendWin += n
	return nil
}
...
func (s *Stream) ConsumeRecvWindow(n int64) error {
	if n < 0 {
		return errors.New("negative receive consumption")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if n > s.RecvWin {
		return fmt.Errorf("receive window exceeded: need=%d available=%d", n, s.RecvWin)
	}
	s.RecvWin -= n
	return nil
}
...
func (s *Stream) AddRecvWindow(n int64) error {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.RecvWin > MaxWindow-n {
		return errors.New("receive window overflow")
	}
	s.RecvWin += n
	return nil
}
...
func (s *Stream) CloseLocal() error {
	...
	default:
		return errors.New("invalid local close transition")
	}
	return nil
}
...
func (s *Stream) CloseRemote() error {
	...
	case StreamHalfClosedLocal:
		s.State = StreamClosed
	default:
		return errors.New("invalid remote close transition")
	}
	return nil
}
